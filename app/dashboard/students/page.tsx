...
export function CourseTabs() {
  ...
  const isCoordinator = user?.roles.some((r) =>
    ['admin', 'coordinator'].includes((r || '').toLowerCase())
  );

  const ptProfile = ptTeachers.find(
    (t) =>
      t.user.email?.toLowerCase() === user?.email?.toLowerCase() ||
      t.user.name?.toLowerCase() === user?.username?.toLowerCase()
  );
...
export default function StudentsPage() {
  ...
  async function onSubmit() {
    ...
    const courseForUpload = (searchParams.get('course') || selectedCourse || '')
      .trim()
      .toLowerCase();

    if (!['informatica', 'electronica'].includes(courseForUpload)) {
      toast.error(
        'Curso inválido para upload. Selecione Informática ou Eletrônica.'
      );
      return;
    }

    uploadCsv(
      { file: files[0], course: courseForUpload },
      {
        onSuccess: () => {
          setFiles([]);
        ...
        <div className=''>
          ...
          {!showDropzone && (
            <div className='space-y-4'>
              ...
              <div className='mt-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between bg-white p-4 r'>
